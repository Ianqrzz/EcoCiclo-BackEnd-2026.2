...
public class AgendamentoController {
    ...
    @GetMapping("/coletor/{coletorId}")
    public ResponseEntity<List<AgendamentoResponseDTO>> listarPorColetor(@PathVariable UUID coletorId) {
        return ResponseEntity.ok(agendamentoService.listarPorColetor(coletorId));
    }

    @GetMapping
    public ResponseEntity<List<AgendamentoResponseDTO>> listarTodos(
            @RequestParam(name = "status", required = false) StatusAgendamento status,
            @RequestParam(name = "coletorId", required = false) UUID coletorId) {
        return ResponseEntity.ok(agendamentoService.listarAgendamentos(status, coletorId));
    }
}
